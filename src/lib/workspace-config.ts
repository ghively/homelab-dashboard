...
/** Query recipe matching for the AI workspace NL composition */
...
export const QUERY_RECIPES: QueryRecipe[] = [
  {
    request: "Show me how X connects to Y",
    workspace: "RelationshipInvestigation",
    tools: ["search_knowledge", "find_relationship_paths", "get_edge_evidence", "explain_relationship_path"],
    components: ["RelationshipPathTopology", "QueryEvidenceCanvas", "CitationTrail", "GraphInspector"],
    keywords: ["connect", "relationship", "link", "depends", "path"],
  },
  {
    request: "Show everything related to X",
    workspace: "ConceptNeighborhood",
    tools: ["search_knowledge", "get_neighbors", "get_related_documents"],
    components: ["ConceptNeighborhood", "RelatedNotesShelf", "BacklinkPanel", "GraphLegend"],
    keywords: ["related", "neighborhood", "around", "connected", "associated"],
  },
  {
    request: "What depends on X?",
    workspace: "BlastRadius",
    tools: ["build_relationship_topology", "get_edge_evidence"],
    components: ["DependencyTopology", "RelationshipMatrix", "QueryEvidenceCanvas"],
    keywords: ["depend", "blast", "radius", "impact", "affected"],
  },
  {
    request: "Open my LLM wiki",
    workspace: "KnowledgeWorld",
    tools: ["list_knowledge_documents", "get_knowledge_document", "get_backlinks", "get_document_outline"],
    components: ["KnowledgeWorldWorkspace", "MarkdownKnowledgeViewer", "WikiNavigationTree", "DocumentOutline", "BacklinkPanel"],
    keywords: ["wiki", "document", "open", "read", "browse"],
  },
  {
    request: "Where did this claim come from?",
    workspace: "Provenance",
    tools: ["search_knowledge", "get_edge_evidence"],
    components: ["ProvenanceGraph", "CitationTrail", "QueryEvidenceCanvas"],
    keywords: ["provenance", "claim", "source", "origin", "evidence", "where"],
  },
];

/** Match a natural-language query to a recipe */
export function matchRecipe(query: string): QueryRecipe | null {
  const q = query.toLowerCase();
  let best: { recipe: QueryRecipe; score: number } | null = null;
  for (const recipe of QUERY_RECIPES) {
    let score = 0;
